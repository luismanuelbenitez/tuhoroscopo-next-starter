...
export async function PATCH(
  ...
  if (body.veredicto !== undefined) {
    patch.veredicto = VEREDICTOS.includes(body.veredicto as typeof VEREDICTOS[number]) ? String(body.veredicto) : null;
  }
  ...
  const res = await fetch(
    `${supabaseUrl}/rest/v1/tarot_benchmark_evaluaciones?id=eq.${encodeURIComponent(params.id)}`,
    {
      method: "PATCH",
