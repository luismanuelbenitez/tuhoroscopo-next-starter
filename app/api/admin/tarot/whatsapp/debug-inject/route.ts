...
// SOLO QA — inyecta un webhook de WhatsApp representativo (schema oficial
// de Meta) sin depender de un número real. Doble gate: sesión admin (acá) +
// x-internal-key (ef_tarot_debug_whatsapp_inbound). Ver docs/modules/whatsapp-inbox.md.
export async function POST(req: NextRequest) {
  ...
  let res: Response;
  try {
    res = await fetch(`${env.supabaseUrl}/functions/v1/ef_tarot_debug_whatsapp_inbound`, {
      method: "POST",
      headers: {
